...
| `.claude/hooks/block-main-push.py` | PreToolUse hook on Bash | Enforces "no pushes to `main`": inspects every `git push` command and blocks any push whose destination is `main`/`refs/heads/main` — direct pushes, refspecs (`HEAD:main`, `+HEAD:main`, `refs/heads/main`, delete), `--force`/`--force-with-lease`, any remote name, and bare `git push` while `main` is checked out. Pushes to `feature/*` and `fix/*` branches remain allowed. No network access, no database logic. |
