...
**Migration:** `supabase/migrations/20260918143850_add_reporting_classification.sql` — **⏳ NOT YET applied to production** (`alxlbdhpbwlehbdbfejw`). To be applied manually through the Lovable Cloud SQL editor after verifying the project ref; the generated `src/integrations/supabase/types.ts` is regenerated from the production schema *after* that, not in this PR. Independent of 6C-b8 (it only touches `public.grootboekrekeningen`, created 20260412); the file sorts before the future-dated `20260919120000` because the CLI stamped it with the real UTC time. That is irrelevant for the SQL editor (which does not consult `schema_migrations`); **apply it through the SQL editor, do not use `supabase db push`** — the CLI would refuse a version older than the newest remote one without `--include-all`, and the remote history does not track the hand-applied migrations anyway.
