...
- **Tests:** `opening-balance-utils.test.ts` (60), `beginbalans-page.test.tsx` (40, hooks mocked with a re-render store so refetches are observable), `opening-balance-posting-action.test.tsx` (24, real QueryClient + mocked Supabase: exactly one RPC per action, no success without the marker/header re-read, invalidation keys, 23505/40001/40P01/42501/22023/PGRST paths for post and nil), `grootboek-combobox-client-scope.test.tsx` (2, real combobox) and `opening-balance-ui-scope.test.ts` (12, static: no `journal_entries`, no document totals, no direct ledger/marker/nil writes, lines only via the save RPC, no migration/SQL/types/package change on the branch, route nested under Grootboek, `nav.ts` untouched).
