...
- **Earliest fact.** Posting is refused when the administratie already has any `ledger_postings` row before `opening_date`: the group would balance, so no later check could ever detect the double count.
...
- **Tests:** `src/test/opening-balance-posting.test.ts` (47 structural assertions over the migration text) plus a real-PostgreSQL proof under `supabase/tests/opening-balance/` (`run-proof.sh`, 89 proofs including six two-session concurrency races). The proof is deliberately **not** part of `npm run test`: CI has no PostgreSQL. It creates and drops a throwaway database and must never be pointed at a BoekAssist database.
