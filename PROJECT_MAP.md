...
- **Afsluiten kan alleen na `ready`.** `blocked`, `incomplete` én `warning` bieden geen knop aan. Die laatste is bewust: PR 1 legde vast dat een waarschuwing géén gereedheid is, en dat blijft staan ook al blokkeert de databaseschrijver bijvoorbeeld niet op een onvolledige beginbalans. **Het scherm is strenger dan het minimum dat de database afdwingt, nooit soepeler.** Er is geen overrideknop.
