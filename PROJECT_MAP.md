...
- **Een afgebroken reeks partijen liegt niet.** Boven `BANK_BULK_MAX_BATCH` knipt de hook de lijst; faalt een latere partij, dan draagt `BankBulkPartialError` de reeds ontvangen resultaten mee. Het scherm toont wat wél is verwerkt, hoeveel regels niet zijn aangeboden, en dat er niets automatisch opnieuw wordt geprobeerd. Er is geen herkansingsmechanisme toegevoegd.
