...
**Migration:** `supabase/migrations/20260919120000_add_opening_balance_posting.sql` — **NOT yet applied to production.**
...
- **Earliest fact, from both sides.** Posting is refused when the administratie already has any `ledger_postings` row before `opening_date` — the group would balance, so no later check could ever detect the double count. The mirror image is closed too: a separate trigger on `ledger_postings` refuses any row dated before a **posted** beginbalans of that administratie, whatever its source. It is inert for every administratie that has no posted beginbalans, which today is all of them. The one race the pair does not close (a beginbalans and a back-dated document committing in the same instant) is stated in the migration rather than papered over; closing it would mean serialising all posting per administratie, which is an owner decision.
...
- **Tests:** `src/test/opening-balance-posting.test.ts` (50 structural assertions over the migration text, comments stripped first) plus a real-PostgreSQL proof under `supabase/tests/opening-balance/` (`run-proof.sh`, **103 proofs** including nine two- and three-session concurrency races). The proof is deliberately **not** part of `npm run test`: CI has no PostgreSQL. It creates and drops a throwaway database and must never be pointed at a BoekAssist database.
- **Explicitly out of scope:** UI, reporting labels and completeness states (later PRs), CSV import, the reversal engine, Balans/W&V, year-end closing, result allocation and the equity/liability split inside `passiva`.

---

Future phases: 6C-b6 manual journal posting is the section above (PR 1 schema + writer, PR 2 application layer — both done), 6C-b7 is the section above (PR 1 reporting core, PR 2 Grootboeksaldi and PR 3 proef- en saldibalans done; Balans/W&V wait for an opening-balance phase), and 6C-b8 PR 1 adds the opening-balance schema and writer (migration not yet applied). Source-level idempotency is handled per writer (purchase, sales, bank and manual journal each guard their own `source_type` on `ledger_postings`) — see the 6C-b2 migration header for why no universal uniqueness constraint is safe.
