...
- **Tabel:** dichtere regels (`px-3 py-1.5` in plaats van de standaard `p-4`), met een duidelijke hiërarchie rekeningregel → subtotaal → eindtotaal. Groepskoppen staan in een eigen band en zijn een `<th scope="colgroup">`, zodat een schermlezer ze als kop van hun blok aankondigt in plaats van als lege cel. Bedragen blijven rechts uitgelijnd, `font-mono`, `tabular-nums` en `whitespace-nowrap`; lange rekeningnamen truncaten met een `title`-tooltip en de volledige naam in het `aria-label`.
...
- **Ongewijzigd in betekenis:** systeemregels houden hun badge "Berekend", hebben geen rekeningnummer en krijgen **nooit** een drilldown — maar ze zijn bewust rustiger opgemaakt dan een subtotaal, want ze zijn toelichting en geen bron. "Niet geclassificeerd" blijft **altijd uitgeklapt**, met focus- en hoverstaten op de doorklikbare rijen. De vier volledigheidsstatussen houden hun tekst en toon; alleen "Volledig" is groen.
- **Tests:** `src/test/financial-statements-ui-polish.test.tsx` (15) pint de engine-waarden per regel en per totaal vast, de vier statustonen, de scrollcontainers, de rechtsuitlijning met tabulaire cijfers, de semantische tabelstructuur (`thead`/`tbody`/`tfoot` en de `colgroup`-kop) en de scope (rekenlagen ongewijzigd, geen dependency/migratie/types/route).
