...
- **`afgesloten_boekjaar` is een WATERMERK, geen los jaar.** Alle zeven schrijvers toetsen met `v_boekjaar <= v_client.afgesloten_boekjaar`; `NULL` betekent dat er nog niets is afgesloten. Sinds 6C-b10 wordt het uitsluitend gezet door `public.close_fiscal_year()`; het handmatige veld op `/klanten` is vervangen door een alleen-lezen weergave.
...
- **Waarschuwingen zijn bewust géén `ready`.** Er bestaat in dit product geen regel die zegt dat open werk of een onvolledige beginbalans een schone uitslag oplevert, dus die regel is hier niet verzonnen: de status is dan `warning`, niet `ready`. Dat is een uitspraak over de **gereedheid**, niet over de toegang tot de afsluitknop — sinds de rookproef op productie mag een erkende waarschuwing wél doorgaan naar de schrijver (zie PR 2b hieronder).
...
### 6C-b10 — de jaarafsluiting (schrijver, PR 2a)

```
supabase/migrations/20260924120000_add_year_close_writer.sql
supabase/tests/year-close/run-proof.sh      42 bewijzen tegen een echte PostgreSQL
src/test/year-close-writer.test.ts          contract over de migratie + de app
```

- **De afsluiting maakt GEEN grootboekboeking.** Geen resultaatbestemming en geen doorrol, en dat is een gevolg van hoe de rapportage al werkt. `financial-statements.ts` sluit de balans met twee **synthetische presentatieregels**: `prior_years_result` (= `pnlOpeningCents`) en `current_year_result` (= `pnlMovementCents`). Een resultaatboeking gedateerd ín jaar N zou in de W&V-mutatie van jaar N vallen — de kern filtert op datum, `boekjaar` filtert nooit — en het gerapporteerde resultaat van het afgesloten jaar dus op nul zetten; gedateerd in N+1 tussen balansrekeningen zou zij dubbeltellen tegen `prior_years_result`. En een doorrol is overbodig: `openingCents` is per definitie Σ(debet − credit) vóór de periodegrens, dus de openingspositie van N+1 ís het cumulatieve grootboek.
- **`public.year_closures` is het bewijs.** `(client_id, fiscal_year)` is de primary key en daarmee de idempotentiegarantie. Append-only (UPDATE, DELETE én TRUNCATE geweigerd, `ENABLE ALWAYS`), tenantintegriteit via `posting_client_org_ok()` in plaats van alleen RLS, en applicatierollen mogen uitsluitend lezen.
- **Geen `result_cents`, bewust.** Het zou een tweede boekhoudmotor vereisen: `grootboekrekeningen.statement_type` is nullable, dus een SQL-som over `winst_verlies` wijkt stilzwijgend af van de motor zodra één rekening ongeclassificeerd is (die behandelt dat geval met `unclassifiedClosingCents` en `coverage_mismatch`). En het is niet nodig: het grootboek van een afgesloten jaar ligt vast, dus het resultaat is altijd reproduceerbaar — terwijl een momentopname juist zou verouderen als de classificatie later wordt gecorrigeerd.
- **Het watermerk beweegt alleen nog mét bewijs.** Een trigger op `clients` laat `afgesloten_boekjaar` alleen naar een jaar waarvoor een `year_closures`-rij bestaat, en nooit omlaag of naar `NULL`. Daarmee is "watermerk vooruit, bewijs ontbreekt" geen toestand die de database nog kan aannemen — structureel in plaats van procedureel. Bestaande, met de hand gezette watermerken blijven werken; alleen het opnieuw afsluiten van zo'n jaar loopt fail-closed vast.
- **De database herkeurt zelf**, onder `lock_ledger_client()` + `SELECT … FOR UPDATE`: watermerkconsistentie, afsluitvolgorde, ongebalanceerde boekingsgroepen t/m het doeljaar, postbaar-maar-ongeboekt bronwerk t/m het doeljaar, en bronwerk zonder boekhoudkundige datum (fail closed). De postbaarheidsregels zijn letterlijk die van `PURCHASE_POSTABLE_STATUSES` / `SALES_POSTABLE_STATUSES`; het boekjaar komt uit `invoice_date` / `transaction_date` / `posting_date` en **nooit** uit `created_at`.
- **Wat de database bewust NIET blokkeert:** classificatie, subgroepen, administratiebrede bevindingen die niet aan het doeljaar zijn toe te wijzen, later bronwerk — en **de beginbalans**. Afsluiten maakt een beginbalans t/m jaar N inderdaad voorgoed onmogelijk (ook de nil-verklaring), en dat hoort op het scherm te staan; het is alleen geen databaseblokkade, omdat er geen invariant bestaat die zegt dat een administratie een beginbalans móét hebben — een administratie die werkelijk in jaar N begint heeft er terecht geen.
...
```
src/lib/year-close-action.ts        puur: mag het, wat betekent de fout, wat staat er op het bewijs
src/hooks/useYearClose.ts           useYearClosure · useCanCloseFiscalYear · useCloseFiscalYear
src/pages/Jaarafsluiting.tsx        /overzichten/jaarafsluiting — nu ook de afsluitactie
```

- **`blocked` en `incomplete` bieden geen knop aan; `warning` wél — geïnformeerd.** Een waarschuwing is advies, geen financiële invariant: de databaseschrijver blokkeert er bewust niet op (zie de lijst "bewust NIET herkeurd" hierboven). De gereedheidsstatus blijft amber en wordt nergens als schoon gepresenteerd, maar de afsluitknop verschijnt. **Afsluiten kan dus met openstaande waarschuwingen, nooit met een blokkade of een onvolledige controle.**
  - Aanleiding: een rookproef op productie — AA-Secure, boekjaar 2023, twaalf controles, tien akkoord, **nul blokkades** — kon niet worden afgesloten om twee waarschuwingen die de schrijver zelf niet tegenhoudt (een ontbrekende beginbalansbewering en negen rekeningen met een categorie maar nog geen rapportagegroep). Dat maakte het scherm niet voorzichtig maar onbruikbaar, met als enige uitweg presentatiewerk dat met de juistheid van het jaar niets te maken heeft.
  - **Er is geen overrideknop en geen verborgen vinkje.** De waarschuwing zélf is het geïnformeerde pad: `unresolvedWarnings()` zet de bevindingen uit de vastgezette momentopname — titel én samenvatting, letterlijk — in de bevestigingsdialoog onder "Nog openstaande waarschuwingen", met de mededeling dat zij niet blokkeren en dat bij bevestigen opnieuw de financieel noodzakelijke voorwaarden worden gecontroleerd. Daarna volstaat dezelfde ene bevestigknop.
  - **Geen enkele controle heeft een andere ernst gekregen**: een ontbrekende rapportagegroep blijft `warning`, de beginbalansvolledigheid houdt haar huidige ernst, en de schrijver is niet aangeraakt. Dit veranderde alleen of een erkende waarschuwing dóór mag naar de database.
- **De database blijft de autoriteit.** Er gaan precies twee waarden de deur uit: `_client_id` en `_fiscal_year`. Geen resultaatbedrag, geen gereedheidsuitslag, geen classificatie, geen watermerk. De rolcontrole op het scherm (`has_min_role`) bepaalt alleen wat er wordt aangeboden; de RPC toetst hoe dan ook opnieuw.
- **Idempotent bewijs.** `created = false` is geen fout maar een mededeling ("Dit boekjaar was al afgesloten. Het bestaande afsluitbewijs is geladen.") met hetzelfde bewijs eronder. Dat is precies het geval na een time-out.
- **Een onbekende afloop wordt eerst verzoend, nooit herhaald.** Bij een netwerkstoring zegt het scherm eerlijk dat niet bekend is of het jaar is afgesloten, haalt het bewijs opnieuw op, en toont dát — ligt er bewijs, dan is het een geslaagde afsluiting; ligt er niets, dan mag de gebruiker opnieuw proberen.
- **De erfenistoestand heeft een eigen melding.** Staat `clients.afgesloten_boekjaar` al op dit jaar terwijl er geen `year_closures`-rij is — administraties waar het veld vóór PR 2a met de hand is gezet — dan zegt het scherm dat expliciet en adviseert het de administratie te laten controleren. Er wordt geen marker verzonnen, geen watermerk verlaagd en niets automatisch herhaald.
- **Ruwe databasetekst komt nooit op het scherm.** `classifyYearCloseError()` vertaalt de negen bekende uitkomsten naar vaste Nederlandse zinnen met een advies. De zes boekhoudkundige weigeringen delen SQLSTATE 23514, dus de soort volgt uit de tekst — en de volgorde is load-bearing: "boekhoudkundige datum onbekend" bevat zélf de woorden "postbaar brondocument" en moet dus éérst worden getoetst.
- **Geen bedrag op het afsluitbewijs.** `year_closures` bewaart geen `result_cents`; een berekening erbij verzinnen zou de tweede boekhoudmotor zijn die PR 2a juist heeft vermeden. `closed_by` wordt alleen benoemd als het de ingelogde gebruiker zelf is — dit leesmodel kent geen gebruikersnamen, en een rauwe uuid is geen informatie.
- **Schermtoestand draagt administratie én boekjaar.** Wisselt een van beide, dan verdwijnt de oude gereedheid en het oude bewijs in plaats van mee te verhuizen.
- **Geen heropening, geen resultaatboeking, geen doorrol, geen migratie.** Deze fase voegt geen SQL toe.

**Twee feiten uit PR 1 die zijn blijven staan — maar niet als openstaande eis:**
