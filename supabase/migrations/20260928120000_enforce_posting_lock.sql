-- ═════════════════════════════════════════════════════════════════════════════
-- 6C-b11 (PR D) — DE BOEKINGSBLOKKADE WORDT GEHANDHAAFD
--
-- Zie docs/BOEKASSIST_YEAR_CLOSE_LIFECYCLE.md, secties C, E2 en H, en de
-- fundering in 20260927120000_add_posting_lock_foundation.sql (PR C).
--
-- WAT HIER GEBEURT
--   PR C legde `clients.posting_locked_through`, `posting_lock_events`,
--   `posting_allowed()` en `set_posting_lock()` aan, maar GEEN schrijver
--   toetste de blokkade. Deze migratie zet die handhaving aan voor elke
--   GEDATEERDE financiële schrijver:
--
--     post_purchase_invoice   → factuurdatum
--     post_sales_invoice      → factuurdatum
--     post_bank_allocation    → transactiedatum van de bankregel
--     post_manual_journal     → boekingsdatum
--     post_opening_balance    → openingsdatum (bestaande datumsemantiek)
--     reverse_posting_group   → datum van de TEGENBOEKING
--     post_bank_transaction   → transactiedatum
--
--   en laat de read-only preflight `bank_bulk_posting_candidates()` dezelfde
--   regel melden, zodat de lijst niet iets als boekbaar toont wat de schrijver
--   daarna weigert.
--
-- TIJDELIJK DUBBELE GRENDEL — MET OPZET
--   De oude toets op `clients.afgesloten_boekjaar` blijft in ELKE schrijver
--   ongewijzigd staan, met dezelfde tekst en dezelfde SQLSTATE. Wat vóór PR D
--   geblokkeerd was, is dat erna nog steeds. Een boeking moet nu door BEIDE
--   poorten: het jaarwatermerk én de datumblokkade.
--
-- DE UITZONDERING: declare_opening_balance_nil()
--   Bewust NIET aangesloten. Zij schrijft geen grootboekregel; zij is een
--   uitspraak over een BOEKJAAR ("er is geen beginbalans"). De juiste
--   beheersmaatregel daarvoor is de boekjaarstatus, niet de datumgebonden
--   boekingsblokkade. Zie sectie E2 van het ontwerpdocument. Haar bestaande
--   afgesloten-jaar-toets blijft onveranderd.
--
-- ÉÉN GEDEELDE ASSERTIE
--   `public.assert_posting_allowed(uuid, date)` — één tekst, één SQLSTATE
--   (22023), zeven aanroepplaatsen. Zij:
--     1. grendelt eerst de administratie via `lock_ledger_client()`, zodat een
--        gelijktijdige `set_posting_lock()` (die dezelfde grendel neemt) er
--        niet tussendoor kan glippen tussen toets en boeking;
--     2. toetst via `posting_allowed()`;
--     3. noemt de blokkadedatum pas NADAT de schrijver de administratie al
--        heeft geautoriseerd — er komt dus geen nieuw tenant-orakel bij.
--
--   Over de grendelvolgorde: `post_opening_balance` en `reverse_posting_group`
--   namen de administratiegrendel al zelf, vóór hun jaartoets; daar verandert
--   niets (een advisory lock is her-ingang-vast). De overige vijf kregen hem
--   voorheen pas bij de eerste INSERT in `ledger_postings`
--   (lock_ledger_client_trigger); zij nemen hem nu iets eerder. Bestaande
--   grendels zijn niet verplaatst.
--
-- WAT DEZE MIGRATIE NIET DOET
--   Geen tabelwijziging, geen aanpassing aan fiscal_year_events,
--   posting_lock_events, year_closures of close_fiscal_year(), geen heropenen,
--   geen nieuwe rechten op tabellen, geen wijziging van rolvloeren en geen
--   enkele wijziging aan de bedragen-, BTW- of debet/credit-logica van de
--   schrijvers. De functielichamen zijn letterlijk die van vandaag, met één
--   toegevoegde regel per schrijver.
--
-- ROLLBACK (handmatig): herdefinieer de acht functies uit hun oorspronkelijke
--   migraties (20260915140000, 20260915160000, 20260917120000, 20260918120000,
--   20260919120000, 20260921120000, 20260921140000, 20260922120000) en
--   daarna: DROP FUNCTION IF EXISTS public.assert_posting_allowed(uuid, date);
-- ═════════════════════════════════════════════════════════════════════════════
...
BEGIN
  IF to_regprocedure('public.posting_allowed(uuid,date)') IS NULL THEN
    RAISE EXCEPTION 'PR D vereist eerst public.posting_allowed(uuid,date) (PR C, 20260927120000)';
  END IF;
  IF to_regprocedure('public.lock_ledger_client(uuid)') IS NULL THEN
    RAISE EXCEPTION 'PR D vereist eerst public.lock_ledger_client(uuid) (6C-b8)';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'clients'
      AND column_name = 'posting_locked_through'
  ) THEN
    RAISE EXCEPTION 'PR D vereist eerst clients.posting_locked_through (PR C)';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'clients'
    ...
    RAISE EXCEPTION 'PR D vereist clients.afgesloten_boekjaar: de oude grendel blijft staan';
  END IF;
END
$migratie$;

-- ── 1. De gedeelde assertie ─────────────────────────────────────────────────
--
--    SECURITY DEFINER, omdat zij `lock_ledger_client()` en `posting_allowed()`
--    aanroept; beide zijn uitsluitend voor de eigenaar uitvoerbaar. VOLATILE,
--    want zij neemt een grendel. Vast search_path. Geen EXECUTE voor anon,
--    authenticated of service_role: alleen de schrijvers (zelf SECURITY
--    DEFINER, zelfde eigenaar) roepen haar aan.
...
BEGIN
  -- Fail closed: zonder administratie of zonder datum valt er niets te toetsen.
  IF _client_id IS NULL OR _posting_date IS NULL THEN
    RAISE EXCEPTION 'Boekingsdatum of administratie ontbreekt; de boekingsblokkade kan niet worden getoetst'
      USING ERRCODE = '22004';
  END IF;

  -- (1) Serialisatie eerst. set_posting_lock() neemt dezelfde grendel, dus na
  --     deze regel kan de blokkade niet meer wijzigen vóór onze COMMIT.
  PERFORM public.lock_ledger_client(_client_id);

  -- (2) Daarna pas oordelen — op de waarde zoals zij ONDER de grendel staat.
  IF public.posting_allowed(_client_id, _posting_date) THEN
    RETURN;
  END IF;

  SELECT c.posting_locked_through INTO v_locked_through
  FROM public.clients c
  WHERE c.id = _client_id;

  -- Bestaat de administratie niet, dan weigert posting_allowed() ook; dan is er
  -- geen blokkadedatum te noemen en blijft de melding leeg van elk feit.
  ...
  RAISE EXCEPTION 'Boekingsdatum % valt binnen de boekingsblokkade t/m % voor deze administratie',
    _posting_date, v_locked_through
    USING ERRCODE = '22023';
END
$$;

REVOKE ALL ON FUNCTION public.assert_posting_allowed(uuid, date)
  FROM PUBLIC, anon, authenticated, service_role;

COMMENT ON FUNCTION public.assert_posting_allowed(uuid, date) IS
'Weigert een boeking waarvan de boekhoudkundige datum binnen de boekingsblokkade van de administratie valt (datum <= clients.posting_locked_through). Neemt eerst lock_ledger_client(), zodat een gelijktijdige set_posting_lock() niet tussen toets en boeking kan glippen. Eén tekst, SQLSTATE 22023, voor alle gedateerde schrijvers. Staat los van clients.afgesloten_boekjaar: die toets blijft in elke schrijver zelf staan.';

-- ── 2. De gedateerde schrijvers ─────────────────────────────────────────────
--
--    Letterlijk de huidige lichamen, met één toegevoegde PERFORM per functie,
--    direct NA de bestaande afgesloten-jaar-toets.
...
BEGIN
  ...
  -- PR D — DE BOEKINGSBLOKKADE. Autoriteit voor deze schrijver: factuurdatum van de inkoopfactuur.
  --        De helper grendelt eerst de administratie (lock_ledger_client),
  --        zodat een gelijktijdige set_posting_lock() er niet tussendoor kan.
  PERFORM public.assert_posting_allowed(v_inv.client_id, v_inv.invoice_date);
...
BEGIN
  ...
  -- PR D — DE BOEKINGSBLOKKADE. Autoriteit voor deze schrijver: factuurdatum van de verkoopfactuur.
  --        De helper grendelt eerst de administratie (lock_ledger_client),
  --        zodat een gelijktijdige set_posting_lock() er niet tussendoor kan.
  PERFORM public.assert_posting_allowed(v_inv.client_id, v_inv.invoice_date);
...
BEGIN
  ...
  -- PR D — DE BOEKINGSBLOKKADE. Autoriteit voor deze schrijver: transactiedatum van de bankregel.
  --        De helper grendelt eerst de administratie (lock_ledger_client),
  --        zodat een gelijktijdige set_posting_lock() er niet tussendoor kan.
  PERFORM public.assert_posting_allowed(v_alloc.client_id, v_tx.transaction_date);
...
BEGIN
  ...
  -- PR D — DE BOEKINGSBLOKKADE. Autoriteit voor deze schrijver: boekingsdatum van het memoriaal.
  --        De helper grendelt eerst de administratie (lock_ledger_client),
  --        zodat een gelijktijdige set_posting_lock() er niet tussendoor kan.
  PERFORM public.assert_posting_allowed(v_journal.client_id, v_journal.posting_date);
...
BEGIN
  ...
  -- PR D — DE BOEKINGSBLOKKADE. Autoriteit voor deze schrijver: openingsdatum van de beginbalans.
  --        De helper grendelt eerst de administratie (lock_ledger_client),
  --        zodat een gelijktijdige set_posting_lock() er niet tussendoor kan.
  PERFORM public.assert_posting_allowed(v_header.client_id, v_header.opening_date);
...
BEGIN
  ...
  -- PR D — DE BOEKINGSBLOKKADE. Autoriteit voor deze schrijver: datum van de TEGENBOEKING, niet die van het origineel.
  --        De helper grendelt eerst de administratie (lock_ledger_client),
  --        zodat een gelijktijdige set_posting_lock() er niet tussendoor kan.
  PERFORM public.assert_posting_allowed(v_client_id, _posting_date);
...
BEGIN
  ...
  --        De helper grendelt eerst de administratie (lock_ledger_client),
  --        zodat een gelijktijdige set_posting_lock() er niet tussendoor kan.
  PERFORM public.assert_posting_allowed(v_tx.client_id, v_tx.transaction_date);
...
-- ── 3. De preflight meldt dezelfde regel ────────────────────────────────────
--
--    De kandidatenlijst is en blijft een WERKSTROOM-classificatie, geen
--    acceptatiemotor: de schrijver oordeelt. Maar zij mag niet `ready` zeggen
--    over een regel die de schrijver gegarandeerd weigert. De blokkade krijgt
--    daarom dezelfde vorm als het jaarwatermerk: `blocked` met een reden, en
--    de rij blijft zichtbaar.
...
--   WHERE n.nspname = 'public' AND p.prosrc LIKE '%assert_posting_allowed%'
--   ORDER BY 1;
--   -- verwacht: de zeven gedateerde schrijvers; NIET declare_opening_balance_nil.
